    ...
    public function index(Request $request)
    {
        try {
            if (auth()->user()->role == 'admin') {
                $consultations = Consultation::whereYear('created_at', now()->year)->get()->load(['visitor:id,name,institution_name,mobile,referral', 'exhibitor:id,company_name']);
            }
            if (auth()->user()->role == 'visitor') {
                $consultations = auth()->user()->consultations()->whereYear('created_at', now()->year)->get()->load(['visitor:id,name,institution_name,mobile,referral', 'exhibitor:id,company_name']);
            }
            if (auth()->user()->role == 'exhibitor') {
                $consultations = auth()->user()->consultations_exhibitor()->whereYear('created_at', now()->year)->get()->load(['visitor:id,name,institution_name,mobile,referral', 'exhibitor:id,company_name']);
            }
            if ($request->status) {
